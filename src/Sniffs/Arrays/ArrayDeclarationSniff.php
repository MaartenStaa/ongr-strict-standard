...
namespace MaartenStaa\OngrStrictStandards\Sniffs\Arrays;
...
class ArrayDeclarationSniff implements Sniff
{
    ...
    public function processSingleLineArray(File $phpcsFile, $stackPtr, $arrayStart, $arrayEnd)
    {
        ...
        if ($valueCount > 0) {
            //ONGR We allow declaring values on a single line
...
//            }
    ...
    public function processMultiLineArray(File $phpcsFile, $stackPtr, $arrayStart, $arrayEnd)
    {
        ...
        $statementStart = $this->getStatementStartColumn($phpcsFile, $stackPtr);
        ...
        } elseif ($tokens[$arrayEnd]['column'] !== $statementStart) {
...
//            }
        ...
        for ($nextToken = ($stackPtr + 1); $nextToken < $arrayEnd; $nextToken++) {
            ...
            if ($tokens[$nextToken]['code'] === T_DOUBLE_ARROW) {
                ...
                // Ongr we require single whitespace before double arrow
                if ($tokens[$nextToken - 1]['code'] !== T_WHITESPACE
        ...
        //Ongr we allow multi-line arrays, that should be single-line
...
//        }
        ...
        */
        //Ongr Multi-line array is not empty and nothing but 1 value was found.
        if (empty($indices)) {
            $value = $arrayStart + 1;
...
//                } else
                if ($tokens[($value['value'] - 1)]['code'] === T_WHITESPACE) {
                    ...
                    $found = $tokens[$value['value']]['column'];
                    if ($found !== $expected) {
...
//                        }
        ...
        */
        ...
        //Ongr we use our own method to find statement start comment.
        $indicesStart  = $statementStart + 4;
...
//            }
     ...
     * @return mixed
     */
    private function getStatementStartColumn(File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
