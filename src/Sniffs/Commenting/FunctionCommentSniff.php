...
if (class_exists(BaseFunctionCommentSniff::class, true) === false) {
    throw new RuntimeException('Class PEAR_Sniffs_Commenting_FunctionCommentSniff not found');
}
...
class FunctionCommentSniff extends BaseFunctionCommentSniff
{
     ...
     */
    protected function processComments(File $phpcsFile, $stackPtr, $commentStart)
    {
        $tokens = $phpcsFile->getTokens();
        $empty = array(
...
//                $phpcsFile->addError($error, $tag, 'EmptyThrows');
     ...
     * @return void
     */
    protected function processDeprecated(File $phpcsFile, $stackPtr, $commentStart)
    {
        $tokens = $phpcsFile->getTokens();
        foreach ($tokens[$commentStart]['comment_tags'] as $pos => $tag) {
